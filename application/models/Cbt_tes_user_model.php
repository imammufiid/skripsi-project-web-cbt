...
class Cbt_tes_user_model extends CI_Model{
	...
	*/
	function get_datatable($start, $rows, $tes_id, $grup_id, $urutkan, $tanggal, $keterangan){
		...
		$this->db->select('cbt_tes_user.*,cbt_user_grup.grup_nama, cbt_tes.*, cbt_user.*, SUM(`cbt_tes_soal`.`tessoal_nilai`) AS nilai, SUM(cbt_tes_soal.tessoal_human_point) AS human_rate')
                 ->where('( '.$sql.' )')
                 ->from($this->table)
    ...
    function get_datatable_evaluasi($start, $rows, $tes_id, $urutkan){
        ...
        $this->db->select('cbt_tes_soal.tessoal_id, cbt_tes_soal.tessoal_jawaban_text, cbt_tes.*, cbt_soal.*, cbt_user.user_firstname')
